...
aws s3 cp $lastlogzipfilename7days s3://moveup-prod-log/prod1/

#get all file from s3
i=0
#**** path (prod1) need edit ****
for aname in $(aws s3 ls s3://moveup-prod-log/prod1/|awk '{print $4}')
do
    filelist[${i}]=$aname
    let i=${i}+1
    #echo $aname
...
do
    ...
    if [ "$count" -gt "30" ]; then
        #**** path (prod1) need edit ****
        aws s3 rm s3://moveup-prod-log/prod1/${filelist[${i}]}
    fi
done
